...
import { modelMeshes, toBinarySTL } from '../export/mesh.js';
...
export class ExportDialog {
  ...
  open() {
    this.app.build();
    this.nameInput.value = this.app.doc.export.filename;
    this.refresh();
  ...
  async run() {
    ...
    const base = safeName(this.nameInput.value || this.app.doc.export.filename, 'text');
    try {
      if (this.format === '3mf') {
        const bytes = await export3MF(modelMeshes(m), { title: this.app.doc.texts[0]?.text.split('\n')[0] || 'Text' });
        downloadBlob(new Blob([bytes], { type: 'model/3mf' }), `${base}.3mf`);
      } else if (this.format === 'stl') {
        downloadBlob(new Blob([toBinarySTL(modelMeshes(m))], { type: 'model/stl' }), `${base}.stl`);
