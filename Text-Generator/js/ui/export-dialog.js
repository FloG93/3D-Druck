...
const INFO = {
  ...
  step: (m) => `Für Fusion 360 und andere CAD-Programme: Jedes Teil (${m.parts.map((p) => `<b>${p.name}</b>`).join(', ')}) ist ein eigenes Bauteil mit Körpern in seiner Farbe, die Umrisse sind exakte Kurven – zum Weiterkonstruieren.${m.cup ? ` Beim Becher liegen Wand und Schrift auf echten ${m.cup.conical ? 'Kegel' : 'Zylinder'}flächen; der Boden reicht bis an die Innenseite der Wand – mit <i>Ändern → Kombinieren</i> werden beide ein Körper.` : ''}
    <br><b>In Fusion 360:</b> <i>Datei → Öffnen → Von meinem Computer öffnen</i> – oder die Datei in den Datenbereich hochladen und per Rechtsklick <i>In aktuelles Design einfügen</i>.${m.stamp?.handle ? ' Der Griff liegt wie im 3MF kopfüber neben dem Stempel.' : ''}${m.stamp?.draft ? ' Die schrägen Flanken sind echte Schrägen (wo ein Buchstabe dabei zuwächst, feine Stufen); Buchstaben, deren Füße zusammenwachsen, überschneiden sich als eigene Körper – mit <i>Kombinieren</i> vereinen.' : ''}`,
  dxf: (m) => (m.relief === 'cut'
